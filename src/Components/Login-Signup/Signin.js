...
class Signin extends Component {
  ...
  handleSignup = () => {
    this.props.signupClicked(this.state);
  }
            ...
            <Grid.Row columns={2} className={"RightSide"}>
              <Grid.Column>
                <Form className={"Form"}>
                  ...
                  <div className={"Test"}>
                    <Button className={"Button Register"} circular inverted color={"green"} basic onClick={this.handleSignup}>register</Button>
                  </div>
                  <Form.Field fluid className={"FormEntry Top"}>
                      <Form.Input type={"text"} name='email' value={email} placeholder={"email"} onChange={this.handleChange}/>
                    </Form.Field>
